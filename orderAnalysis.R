...
#                             '2015/6/1' and '2015/6/10'  and fact_1234 = '1'  and bomseq >= 0  and 
...
#                             ")
# save.image("C:/Users/David79.Tseng/Dropbox/David79.Tseng/git-respository/orderAnalysis/order_20150601_20150610.RData")
...
sectorClassified <- function(sector){
  ...
  if(sector %in% c('aonline', 'ia-aonline')){
    out <- "IAAOnline"
    ...
    out <- "iSys_CSF"
  }else if(sector %in% c('isys-ka', 'icloud-bcd', 'icloud-napd')){
    out <- "iSystemKA"
    ...
    out <- "iServiceAOnline"
  }else if(sector %in% c('medical equipment builder', 'ihospital')){
    out <- "DHealthcare"
  }else if(sector %in% "d. logistic"){
...
modePayment <- function(sector){
  sector <- tolower(sector)
  if (sector %in% c("ia-ka", 'isys-ka', 'icloud-bcd', 'icloud-napd', 'ec-ka', 'ec-ka2', 'ec-ka3', 'mbds-dtos', 'com', 'ec-dms', 'ms', 'medical equipment builder', 'is-ka')){
    out <- "KA"
  }else if(sector %in% c('aonline', 'ia-aonline', "isys-aonline", "ec-aonline", "is-aonline")){
    out <- "AOnline"
  }else if (sector %in% c("ia-csf", "isys-csf", "ec-csf", 'ihospital', 'is-csf')){
    out <- "Channel"
  }else{
    out <- "Others"
  }
...
## new version
OrderSeason <- sapply(1:nrow(dataCleaning), function(i){
  seasonDefine(as.numeric(strsplit(as.character(dataCleaning$OrderDate[i]), split = "-")[[1]][2]))
})
ModePayment <- sapply(1:nrow(dataCleaning), function(i){
  modePayment(dataCleaning[i, "Sector"])
})
NewSector <- sapply(1:nrow(dataCleaning), function(i){
  sectorClassified(dataCleaning[i, "Sector"])
})
TimeDiff <- sapply(1:nrow(dataCleaning), function(i){
  d <- dataCleaning[i, ]
  if (strptime(d$OrderDate, "%Y-%m-%d") - strptime(d$YMD, "%Y-%m-%d") >= 0){
    timediff <-  0
  }else{
...
LeadTime <- sapply(1:nrow(dataCleaning), function(i){
  d <- dataCleaning[i, ]
...
head(dataCleaning)
table(dataCleaning$Region)
##
#dataForUse <- dataCleaning[, c("Cust", "RBU", "Region", "PG", "PD", "Sales.id", "Qty", "US.Amt", "NewSector", "Delay")]
dataForUse <- dataCleaning[, c("Region", "PG", "Qty", "US.Amt", "ModePayment", "NewSector", "OrderSeason", "Delay")]
...
tree1 <- rpart(Delay~., data = dataForUse, method = "class")
plot(tree1)
text(tree1)
...
my_forest <- randomForest(Delay ~., data = dataForUse, importance = TRUE, ntree = 1000)
my_forest <- randomForest(Delay ~., data = dataForUse, importance = TRUE, ntree = 1000, sampsize = c(100, 100), na.action = na.omit)
my_forest
...
nrow(dataForUse)
length(unique(dataForUse$Cust))
length(unique(dataForUse$Part))
length(unique(dataForUse$RBU))
length(unique(dataForUse$PG))
length(unique(dataForUse$PD))
length(unique(dataForUse$Sales.id))
length(unique(dataForUse$DRC))
length(unique(dataForUse$NewSector))
